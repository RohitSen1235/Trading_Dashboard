...
def dashboard(request):
    ...
    if request.method == "POST":
        ...
        if 'excelFile' in request.FILES:
            ...
            content['opening_balance'] = balances[0]
            content['closing_balance'] = balances[1]
            content['profit'] = np.round(balances[1] - balances[0],2)
            content['percentage_gain'] = np.round((balances[1] - balances[0])*100/balances[0], 2)
            content['profitable_tickers'] = data.get_most_profitable_tickers()['ProfitLoss']
            # print(data.head(10))
